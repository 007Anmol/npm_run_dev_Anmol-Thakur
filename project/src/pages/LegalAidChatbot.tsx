...
import { MessageSquare, Send, Info, ThumbsUp, ThumbsDown, ExternalLink, FileText, MapPin, User } from 'lucide-react';
...
const LegalAidChatbot = () => {
  ...
  const [mode, setMode] = useState<'general' | 'notice' | 'roadmap' | 'referral'>('general');
  ...
  const generatePrompt = (conversation: Message[], currentMode: string = mode) => {
  ...
  const extractSources = (text: string) => {
    ...
    const citationRegex = /\[(Section|Chapter|Article|Rule)\s+(\d+[A-Za-z]?)\s+of\s+(.*?)\]/g;
    while ((match = citationRegex.exec(text)) !== null) {
      ...
      const url = `https://indiacode.nic.in/search/${encodeURIComponent(match[3])}`;
  ...
  const detectUserIntent = (userMessage: string): 'general' | 'notice' | 'roadmap' | 'referral' => {
    const lowerMessage = userMessage.toLowerCase();
    
    // Check for legal notice creation request
    if (
      (lowerMessage.includes("draft") || lowerMessage.includes("create") || lowerMessage.includes("write") || lowerMessage.includes("prepare")) && 
      (lowerMessage.includes("notice") || lowerMessage.includes("legal document") || lowerMessage.includes("letter") || lowerMessage.includes("complaint"))
    ) {
      return 'notice';
    }
    ...
    if (
      (lowerMessage.includes("procedure") || lowerMessage.includes("process") || lowerMessage.includes("steps") || 
       lowerMessage.includes("how to") || lowerMessage.includes("roadmap") || lowerMessage.includes("guide")) &&
      (lowerMessage.includes("file") || lowerMessage.includes("court") || lowerMessage.includes("legal") || 
       lowerMessage.includes("case") || lowerMessage.includes("complaint") || lowerMessage.includes("petition"))
    ) {
      return 'roadmap';
    }
    ...
    if (
      (lowerMessage.includes("lawyer") || lowerMessage.includes("attorney") || lowerMessage.includes("advocate") || 
       lowerMessage.includes("legal counsel") || lowerMessage.includes("legal advice")) &&
      (lowerMessage.includes("recommend") || lowerMessage.includes("refer") || lowerMessage.includes("find") || 
       lowerMessage.includes("need") || lowerMessage.includes("contact") || lowerMessage.includes("who"))
    ) {
      return 'referral';
    }
  ...
  const findRelevantLawyers = (query: string): Lawyer[] => {
    ...
    const indianCities = ["delhi", "mumbai", "bangalore", "kolkata", "chennai", "hyderabad", 
                          "ahmedabad", "pune", "lucknow", "jaipur", "chandigarh", "kochi"];
    let location = "";
    ...
    if (location) {
      filteredLawyers = filteredLawyers.filter(lawyer => 
        lawyer.location.toLowerCase() === location.toLowerCase());
    }
  ...
  const formatLawyerReferrals = (lawyers: Lawyer[], query: string): string => {
    ...
    formattedText += "DISCLAIMER: These are synthetic lawyer profiles for demonstration purposes only. In a real application, you would provide actual verified lawyer information. Always verify credentials before engaging legal services.";
